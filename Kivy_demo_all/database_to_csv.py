...
if __name__ == '__main__':
    ...
    cursror.execute(select)

    row= cursror.fetchall()
    column_names = [i[0] for i in cursror.description]
    print(column_names)
    df = pd.DataFrame(row)
    fp = open('out.csv', 'w')
    ...
    fp.close()
    # print(df)
    # df.to_csv('export_data.csv',index=True)  # place 'r' before the path name to avoid any errors in the path
